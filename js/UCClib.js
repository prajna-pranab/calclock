 ...
 * version 1.1.1 13 Scorpi0 13520 - Moved util functions from UCCUtils to here
 * version 1.1.0 13 Scorpi0 13520 - Fixed deek for intercalary days and fixed quarter logic
...
var UCCDate = function() {
  ...
  // lookup tables
  const LANGUAGES = ['Western', 'Hindi', 'Hellenistic', 'Cymraeg'];
  const TRIADS = ['Aries','Taurus','Gemini','Cancer','Leo','Virgo','Libra','Scorpio','Sagittarius','Capricorn','Aquarius','Pisces'];
  const HITRIADS = ['Mesham','Vrishabham','Mithunam','Karkatakam','Simham','Kanya','Thula','Vrischikam','Dhanus','Makaram','Kumbham','Meenam'];
  const GRETRIADS = ['Krios','Tavros','Didymoi','Karkinos','Leōn','Parthenos','Zygos','Skorpios','Toxotēs','Aigokerōs','Hydrokhoos','Ikhthyes'];
  const CWMTRIADS = ['Hwrdd','Tarw','Efeilliaid','Cranc','Llew','Gwyryf','Graddfeydd','Sgorpion','Saethwr','Gafr Fôr','Cludwr Dŵr','Pysgod'];
  const TSYMBOLS = ['\u2648','\u2649','\u264A','\u264B','\u264C','\u264D','\u264E','\u264F','\u2650','\u2651','\u2652','\u2653'];
  const DECANS = ['Neptune','Sol','Mercury','Venus','Earth','Mars','Ceres','Jupiter','Saturn','Uranus'];
  const HICANS = ['Varuna','Ravi','Budha','Shukra','Thal','Mangala','Shakti','Guru','Shani','Vasuki'];
  const GREECANS = ['Poseidon','Helios','Hermes','Aphrodite','Terra','Ares','Demeter','Zeus','Cronus','Caelus'];
  const CWMCANS = ['Poseidon','Helios','Hermes','Aphrodite','Terra','Ares','Demeter','Zeus','Cronus','Caelus'];
  const DSYMBOLS = ['\u2646','\u2609','\u263F','\u2640','\u2295','\u2642','\u26B3','\u2643','\u2644','\u2645'];
  ...
  function trTriad(triad) {
    if (__names == 0) return TRIADS[triad];
    if (__names == 1) return HITRIADS[triad];
    if (__names == 2) return GRETRIADS[triad];
    if (__names == 3) return CWMTRIADS[triad];
  }
  ...
  function zodiac(date) {
    var year = date.year % 24000;
    if (year < 1833)
      return year + ' ' + trTriad(5) + ', Great Summer Descending'; // virgo
    if (year < 3733)
      return (year - 1833) + ' ' + trTriad(4) + ', Great Summer Descending';
    if (year < 5699)
      return (year - 3733) + ' ' + trTriad(3) + ', Great Summer Descending';
    if (year < 7732)
      return (year - 5699) + ' ' + trTriad(2) + ', Great Autumn Descending';
    if (year < 9832)
      return (year - 7732) + ' ' + trTriad(1) + ', Great Autumn Descending';
    if (year < 12000)
      return (year - 9832) + ' ' + trTriad(0) + ', Great Autumn Descending'; // aries
    if (year < 14168)
      return (year - 12000) + ' ' + trTriad(11) + ', Great Winter Ascending';
    if (year < 16268)
      return (year - 14168) + ' ' + trTriad(10) + ', Great Winter Ascending';
    if (year < 18301)
      return (year - 16268) + ' ' + trTriad(9) + ', Great Winter Ascending';
    if (year < 20267)
      return (year - 18301) + ' ' + trTriad(8) + ', Great Spring Ascending';
    if (year < 22167)
      return (year - 20267) + ' ' + trTriad(7) + ', Great Spring Ascending';
    return (year - 22167) + ' ' + trTriad(6) + ', Great Spring Ascending'; // libra
  }
  ...
  function deekDay(date) {
    ...
    if (date.triad == 0 || date.day == 0)
      return '';
    return (__names == 0) ?
      DECANS[(date.day + 10) % 10] :
      (__names == 1) ?
      HICANS[(date.day + 10) % 10] :
      (__names == 2) ?
      GREECANS[(date.day + 10) % 10] :
      CWMCANS[(date.day + 10) % 10];
  }
  ...
  function triadName(date) {
    return (date.triad ? trTriad(date.triad -1) : 'Zero');
  }
  ...
  // extract the Day-of-year number from a ms offset from UCC Epoc
  function msToDay(ms) {
    var doy = msToDoy(ms);
    return doyToDay(doy);
  }
  ...
  function parse(str) {
    ...
    if (str.indexOf('T') != -1) {
      // ISO 8601 specifies that the string represents a UTC date/time unless a timezone is included
      // so this will return a UTC date
  ...
  function outFull(date) {
    ...
    if (date.doy == 1) return "1 ZERO " + date.year;
    var ord = (date.day > 0 ? date.day.toString() : '0');
    return ord + ' ' + numbers[date.triad -1] + '-' + 
      (__names == 0 ?
        TRIADS[date.triad -1] :
        __names == 1 ?
        HITRIADS[date.triad - 1] :
        __names == 2 ?
        GRETRIADS[date.triad - 1] :
        CWMTRIADS[date.triad - 1])
      + TSYMBOLS[date.triad -1] + ' ' + date.year;
  }
  ...
  // create UCC object
  let __instant,  // the actual instant in time represented by the object, as ms offset from UCC Epoc
    __names = 0,  // Language for triad/deek names
    __tzOffset = new Date().getTimezoneOffset() * 60000,  // Local time offset in ms
    __timeZone = 0; // timeZone 0 = UTC, 1 = Local
  ...
  Object.defineProperty(this, 'quarter', {
    get: function () {return (Math.floor(this.triad / 3) + (this.triad % 3 ? 1 : 0))}
  });
  ...
  // ISO 8601 Date represented by this instant as UTC date/time
  Object.defineProperty(this, 'utcDate', 
    {get: function () {return new Date(UCCtoUnix(this.instant))}});

  // ISO 8601 Date represented by this instant. Javascript uses local times, so we
  // offset them to UTC
  Object.defineProperty(this, 'jDate', 
    {get: function () {return new Date(UCCtoUnix(this.instant + __tzOffset))}});
  ...
  // Language for names
  Object.defineProperty(this, 'names', {get: function () {return LANGUAGES[__names]},
                                        set: function (lang) {__names = lang}});

  // Library version
  Object.defineProperty(this, 'version', {get: function () {return VERSION}});
  
  //**************************** Expose our symbol tables ************************//
  Object.defineProperty(this, 'LANGUAGES', {get: function () {return LANGUAGES}});
...
// utility fn to insert a text node into a span element
